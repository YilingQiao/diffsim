...
def run_sim(steps, sim, net, goal):
	...
	for step in range(steps):
		...
		for i in range(len(handles)):
			...
			sim_input = torch.cat([torch.tensor([0, 0],dtype=torch.float64), net_output])
			sim.cloths[0].mesh.nodes[handles[i]].v += sim_input + torch.tensor([0, 0, 0.6],dtype=torch.float64)
